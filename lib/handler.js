...
module.exports = {
    ...
    framework: {
        install: (obj, plugin_dir, project_dir, plugin_id, options) => {
            ...
            // MUST be synchronous — async `execa(...)` without await is
            // fire-and-forget, so the second install (and subsequent
            // framework.install calls for other plugins) race with this
            // one. Observed failure: the appPackage.cordova.services
            // block gets the entry but `dependencies` does not, so at
            // runtime cdv-electron-main.js throws `Cannot find module
            // 'cordova-plugin-file'`. Some plugins happen to finish
            // fast enough; larger ones (bigger src/electron trees) don't.
